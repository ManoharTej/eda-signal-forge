...
export default function ForensicWorkstationMonolith() {
  ...
  /**
   * UNSUPERVISED_ML_RECONSTRUCTION (UMAS V.21)
   * logic: Hyper-sensitive suppression. Anything > 2.0 is forced to 0.8 - 1.0.
   */
  const runMLReconstruction = useCallback((val: number): number => {
    if (val < KERNEL_CONFIG.ARTIFACT_THRESHOLD_μS) return val;
    // Surgical override back to physiological base (0.8 - 1.0 range)
 ...
 const exportClinicalDossier = async () => {
  pushVaultLog("Initializing high-definition clinical capture...", 'SYS');
  
  // 1. Sync check: Ensure we are capturing the static result canvases
  const canvases = document.querySelectorAll('canvas');
  if (canvases.length < 2) {
    pushVaultLog("Error: Analytical traces not yet rendered.", 'CRIT');
    return;
  }
  ...
  // logic: Hospital-grade continuous line capture (No Dots)
  // We use a high quality 1.0 compression to preserve line smoothness
    ...
    return; 
          ...
          if (timeRefMark - lastTableUpdate.current > KERNEL_CONFIG.MATRIX_UPDATE_INTERVAL) {
            ...
            const frame: ForensicDataPoint = {
              ...
              stabilityIndex: edaInput > 2.0 ? 0 : 100, 
              isArtifact: edaInput > 2.0,
              timestamp: new Date().toLocaleTimeString(),
              epoch: timeRefMark
            };
            setTelemetryMatrix(prev => [frame, ...prev].slice(0, 50));
            lastTableUpdate.current = timeRefMark;
            pushVaultLog(`Matrix Latch: ${edaInput.toFixed(4)}μS`, edaInput > 2.0 ? 'WARN' : 'DATA');
          }
        ...
        <div className={`bg-[#0d1117]/95 ${KERNEL_CONFIG.THEME.PANEL_BLUR} border border-slate-800 w-full max-w-7xl rounded-[5rem] p-12 shadow-2xl overflow-hidden relative`}>
           ...
           <div className="grid grid-cols-2 gap-8 mb-12 relative z-10">
              <div className="bg-black/40 p-8 rounded-[3.5rem] border border-slate-800 h-80 shadow-2xl relative overflow-hidden">
                ...
                <Line data={{ labels: telemetryMatrix.map(m => m.timestamp).reverse(), datasets: [{ data: telemetryMatrix.map(m => m.raw_μS).reverse(), borderColor: '#ef4444', borderWidth: 2, pointRadius: 4, fill: true, backgroundColor: 'rgba(239,68,68,0.02)' }] }} options={clinicalChartOptions} />
              </div>
              <div className="bg-black/40 p-8 rounded-[3.5rem] border border-slate-800 h-80 shadow-2xl relative overflow-hidden">
                ...
                <Line data={{ labels: telemetryMatrix.map(m => m.timestamp).reverse(), datasets: [{ data: telemetryMatrix.map(m => m.refined_μS).reverse(), borderColor: '#10b981', borderWidth: 2, pointRadius: 4, fill: true, backgroundColor: 'rgba(16, 185, 129, 0.02)' }] }} options={clinicalChartOptions} />
              </div>
           </div>
           ...
           <div className="bg-black/30 border border-slate-800 rounded-[3rem] p-10 flex items-center justify-between relative shadow-2xl">
        ...
        <div className="flex-[3] flex flex-col gap-4 overflow-hidden">
          <div className={`flex-1 bg-[#0d1117]/80 ${KERNEL_CONFIG.THEME.PANEL_BLUR} border border-slate-800 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl group transition-all duration-1000 hover:border-red-900/40`}>
            ...
            <div className="h-[82%] relative">
              <Line data={{ labels: Array(120).fill(''), datasets: [{ data: phasicRawBuffer, borderColor: '#ef4444', borderWidth: 4, pointRadius: 0, fill: true, backgroundColor: 'rgba(239, 68, 68, 0.025)', tension: 0.15 }] }} options={clinicalChartOptions} />
            </div>
          </div>
        </div>
...
<td className="p-4 text-blue-500 font-black italic leading-none">
  {(f.signalEntropy ?? 0).toFixed(2)}
                      ...
                      <td className={`p-4 text-right font-black italic leading-none ${f.raw_μS > 2.0 ? 'text-red-500' : 'text-emerald-500'}`}>{f.stabilityIndex}%</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* VAULT AUDIT LOG (Rapid-String Hacker Vibe) */}
          <div className={`flex-1 bg-black/90 ${KERNEL_CONFIG.THEME.PANEL_BLUR} border border-slate-800 rounded-[2.5rem] p-10 overflow-hidden shadow-2xl flex flex-col relative group`}>
             ...
             <div className="flex items-center gap-6 mb-8 border-b border-slate-800/30 pb-8 relative z-10">
                <Bolt size={28} className="text-blue-500" />
                <span className="text-[15px] font-black text-white uppercase tracking-widest italic leading-none">Vault_Audit_Kernel</span>
             </div>
             <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-8 z-10">
                {vaultAuditTrail.map((entry, i) => (
                  <div key={i} className="text-[12px] font-bold text-slate-700 uppercase leading-tight tracking-tight hover:text-slate-500 transition-colors duration-300 flex gap-8 group/log">
                    <span className="text-blue-900 font-black italic opacity-30 group-hover/log:opacity-100 transition-opacity">»</span>
                    <span className={entry.lvl === 'CRIT' || entry.lvl === 'WARN' ? 'text-red-500 animate-pulse' : entry.lvl === 'DATA' ? 'text-emerald-500' : 'text-slate-400'}>{entry.msg}</span>
                  </div>
                ))}
             </div>
          </div>
